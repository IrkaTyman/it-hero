...
import { Calendar, Clock, FileText, Link as LinkIcon, LoaderIcon, MapPin, Users } from "lucide-react";
import { findHackathonById } from "@/api/hackathon.ts";
import { useEffect, useState } from "react";

export default function HackathonDetail() {
    const { id } = useParams();
    const { getChallengesByHackathonId, getTimelineByHackathonId } = useData();
    ...
    useEffect(() => {
        findHackathonById(id || "").then (result => setHackathon(result)).catch(() => setHackathon(undefined));
    }, [id])
                            ...
                            <div className="relative border-l border-border pl-8 ml-4 space-y-8">
                                {timeline.map((event, index) => (
                                    <div key={event.id} className="relative">
                                        <div
                                            className="absolute -left-[2.35rem] h-6 w-6 rounded-full bg-primary/20 flex items-center justify-center">
